...
export const uploadPost = async (req, res) => {
    ...
    const folderPath = path.join(__dirname, '../media', folder_path || "");
...
export const changeFilename = async (req, res) => {
    const { folder_path } = req.query;
    const { folder, oldName, newName } = req.body;
    const cleanFolder = folder.replace("/", "");
    const mediaDir = path.join(__dirname, "..", cleanFolder, folder_path || "");
...
const calculateFolderSize = (folderPath) => fs.readdirSync(folderPath, { withFileTypes: true })
    .reduce((acc, item) => {
...
export const getFolder = async (req, res) => {
    try {
        const mediaPath = path.join(__dirname, "..", "media", req.query.folder_path || "");
